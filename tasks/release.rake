  ...
  in_dir(WORKSPACE_DIR) do
    stage('ExtractVersion', 'Extract the last version from CHANGELOG.md and derive next version unless specified', :always_run => true) do
      ...
      unless next_version
        ...
        ENV['PRODUCT_VERSION'] = "#{version_parts[0]}.#{sprintf('%d', version_parts[1].to_i + 1)}"
      end
    ...
    stage('ZapWhite', 'Ensure that zapwhite produces no changes') do
      sh "bundle exec zapwhite --exclude-pattern 'compiler/src/test/fixtures/[^/]+/output/.*'"
    end
...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/gwt-mmvp/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/gwt-mmvp/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/gwt-mmvp', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/gwt-mmvp').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/gwt-mmvp', milestone[:number], :state => 'closed')
        end
      end
    end
  end
